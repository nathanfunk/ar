...
void Win32Timer::reset(float initialTime)
{
	if ( m_usePerformanceCounter == true ){
		QueryPerformanceCounter(( LARGE_INTEGER* )&m_pcTimerStart );

		//fprintf(stderr, "%I64d %3.4f initialtime: %3.4f ", m_pcTimerStart, m_resolution, initialTime);
		m_pcTimerStart-=(__int64) (initialTime/m_resolution);
		//fprintf(stderr, "setting m_pcstart to %3.4f\n", (float) m_pcTimerStart*m_resolution);
	}
	else
		m_mmTimerStart = timeGetTime() + initialTime;

	m_pauseTime = getTime() + initialTime;
}
...
double Win32Timer::getTime() const
{
	...
	if ( m_usePerformanceCounter == true )
	{
		...
		timeElapsed -= m_pcTimerStart;
			//fprintf(stderr, "PerformanceCounter: time = %3.4f\n", (float) timeElapsed*m_resolution);
	...
	{
		fprintf(stderr, "not use performance counter\n");
