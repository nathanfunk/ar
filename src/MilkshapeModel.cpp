	...
	for ( i = 0; i < m_numJoints; i++ )
	{
		...
		if ( strlen( pJoint->m_parentName ) > 0 )
		{
			...
			if ( parentIndex == -1 ) {
				...
				fprintf(stderr, "Unable to find parent bone in MS3D file\n");
		...
		for ( j = 0; j < pJoint->m_numTranslationKeyframes; j++ )
		{
			...
			//fprintf(stderr, "%3.4f ", pKeyframe->m_time*1000.0f);
		}
	}
	delete[] pNameList;
