...
int glbmp_LoadBitmap(const char * bmp_file, int flags, glbmp_t * p_bmp_out)
{
   ...
   do
   {
	  ...
	  if(!_bmp_ReadHeader(&ctx))     {errorCode = -4;              break;}
 
      /* read and validate the bitmap info */
      if(!_bmp_ReadInfo(&ctx, flags))              {errorCode = -5;              break;}
 
      /* get ready for decode */
      if(!_bmp_InitDecode(&ctx))                   {errorCode = -6;              break;}
 
      /* read and convert bitmap data */
      if(!_bmp_Decode(&ctx, flags))                {errorCode = -7;              break;}
