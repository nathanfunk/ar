...
GLuint LoadTGA(char * filename)				// Load a TGA file
{
	...
	{																		// an Uncompressed TGA image
		return LoadUncompressedTGA(filename, fTGA);						// If so, jump to Uncompressed TGA loading code
	}
	else if(memcmp(cTGAcompare, &tgaheader, sizeof(tgaheader)) == 0)		// See if header matches the predefined header of
	{																		// an RLE compressed TGA image
		return LoadCompressedTGA(filename, fTGA);							// If so, jump to Compressed TGA loading code
	}
...
//		MessageBox(NULL, "TGA file be type 2 or type 10 ", "Invalid Image", MB_OK);	// Display an error
	...
	return 0;															// All went well, continue on
}

GLuint LoadUncompressedTGA( char * filename, FILE * fTGA)	// Load an uncompressed TGA (note, much of this code is based on NeHe's 
...
GLuint LoadCompressedTGA(char * filename, FILE * fTGA)		// Load COMPRESSED TGAs
{ 
